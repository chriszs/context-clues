...
function Graph() {
  ...
  function rescale(w, h, parseNodes, parseEdges) {
    ...
    parseNodes && self.nodes.forEach(function(node) {
      ...
      if (!node['fixed']) {
        node['displayX'] = 1.5*(node['x'] - (xMax + xMin) / 2) * scale + w / 2;
        node['displayY'] = .75*(node['y'] - (yMax + yMin) / 2) * scale + h / 2;
      }
    });
...
function Plotter(nodesCtx, edgesCtx, labelsCtx, hoverCtx, graph, w, h) {
  ...
  // TODO: Put all that ^ down below that V, with the rest of the functions.
  ...
  this.p = {
    ...
    //   Label hover size (TODO: implement)
    labelHoverFontSize: 20,
  ...
  /**
   * The canvas context dedicated to accumulating drawn edges.
   * @type {CanvasRenderingContext2D}
   */
  var edgesBufferCtx;
  ...
  /**
   * Set the edgeBufferCtx. 
   * This function is used rather than a constructor for compatibility purposes.
   */
  function setEdgesBufferContext(ctx) {
    edgesBufferCtx = ctx;
  }

  /**
   * Change the canvas context we draw edges to from one to another,
   * alternating between "edges" and "edgesBuffer".
   */
  function swapEdgesCanvasContexts() {
    var ctx = edgesCtx;
    edgesCtx = edgesBufferCtx;
    edgesBufferCtx = ctx;
  } 
  ...
  function drawEdge(edge) {
    ...
    // We always want to draw directly to the buffer canvas
    var ctx = edgesBufferCtx; // edgesCtx;
    ...
    switch (edge['type'] || self.p.defaultEdgeType) {
      case 'curve':
        ...
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.quadraticCurveTo((x1 + x2) / 2 + (y2 - y1) / 8,
                             (y1 + y2) / 2 + (x1 - x2) / 8,
                             x2,
                             y2);
        ctx.stroke();
  ...
  function drawHoverNode(node) {
    ...
    // Font size on hover should increase in order to be legible
    if (fontSize < 14)
      fontSize = self.p.labelHoverFontSize
  ...
  this.setEdgesBufferContext = setEdgesBufferContext;
  this.swapEdgesCanvasContexts = swapEdgesCanvasContexts;
}
...
function Sigma(root, id) {
  ...
  this.visibleEdgesCanvas = this.domElements.edges;
  this.bufferEdgesCanvas = this.domElements.edgesBuffer;
  ...
  this.plotter.setEdgesBufferContext(
    this.domElements.edgesBuffer.getContext('2d'))
  ...
  function draw(nodes, edges, labels, safe) {
    ...
    /*
      if == c, then we have finishd drawing; switch the double buffer canvases and set to zero.
      if it is not equal to c, do not switch canvases and let it continue to whatever it was doing before
    */

    var shouldClearBuffer;

    if (self.plotter.currentEdgeIndex === self.graph.edges.length) {
      // The previous draw call completed drawing all edges to the buffer.
      // Switch the buffer and normal canvii.


      // Swap canvii
      var tempCanvas = this.visibleEdgesCanvas
      this.visibleEdgesCanvas = this.bufferEdgesCanvas
      this.bufferEdgesCanvas = tempCanvas

      // Swap contexts too to keep in sync
      self.plotter.swapEdgesCanvasContexts()

      // Present the buffer on-screen
      this.bufferEdgesCanvas.style.display = 'none'
      this.visibleEdgesCanvas.style.display = ''

      // Reset the edge count
      self.plotter.currentEdgeIndex = 0;
      shouldClearBuffer = true;
    } else {
      // Keep drawing from the current edge index on 
    ...
    for (var k in self.domElements) {
      if (
        ...
        var canvas = self.domElements[k]
        var canvasCtx = canvas.getContext('2d')
        
        // Never clear the visible canvas
        if (canvas === this.visibleEdgesCanvas)
          continue;

        // Do not clear the buffer if we're currently accumulating drawn edges
        if (!shouldClearBuffer && canvas === this.bufferEdgesCanvas)
          continue;
...
sigma.chronos = new (function() {
   ...
   * If several tasks are present, they will be executed in parallel.
  ...
  /**
   * Removes a task.
   * @param  {string} v           If v is undefined, then every task will
